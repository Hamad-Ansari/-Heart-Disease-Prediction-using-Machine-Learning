...
def show_data_preprocessing(df):
    ...
    st.markdown('<div class="subsection-header">Categorical Encoding Mappings</div>', unsafe_allow_html=True)
    
    for feature, mapping in encoding_mappings.items():
        with st.expander(f"Show {feature} encoding"):
...
def show_single_prediction(df_processed):
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            ...
            cp_options = list(encoding_mappings['cp'].keys()) if 'cp' in encoding_mappings else ["typical angina", "atypical angina", "non-anginal", "asymptomatic"]
        ...
        with col2:
            ...
            restecg_options = list(encoding_mappings['restecg'].keys()) if 'restecg' in encoding_mappings else ["normal", "stt abnormality", "lv hypertrophy"]
        ...
        # Additional inputs
        slope_options = list(encoding_mappings['slope'].keys()) if 'slope' in encoding_mappings else ["upsloping", "flat", "downsloping"]
        ...
        thal_options = list(encoding_mappings['thal'].keys()) if 'thal' in encoding_mappings else ["normal", "fixed defect", "reversible defect"]
